...
from model_opp import GlobalDiscriminator
from model_opp import ActivityClassifier
from model_opp import LocalDiscriminator
from model_opp import AttentionNetwork
...
class Solver(object):
    def __init__(self, args):
        ...
        if args.dataset == 'opp':
            self.N = 39
            self.section = [3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,1,3,3,3,3,3,1]
        else:
            self.N = 12
            self.section = [3,3,3,3,3,3, 3,3,3,3,3,3]
        
        self.datasets, self.dataset_test = dataset_read(self.batch_size, self.test_user, self.dataset)
    
        self.FE = [ FeatureExtracter() for i in range(self.N) ]
        self.GD = GlobalDiscriminator()
        self.AC = ActivityClassifier(dataset=self.dataset)   
        self.LD = [ LocalDiscriminator() for i in range(self.N) ]
        self.WG = AttentionNetwork(dataset=self.dataset)
    ...
    def set_optimizer(self, which_opt='momentum', lr=0.001, momentum=0.9):
        if which_opt == 'momentum':
            self.opt_fe = [ optim.SGD(self.FE[i].parameters(),
                                   ...
                                   momentum=momentum) for i in range(self.N) ]
        ...
        if which_opt == 'adam':
            self.opt_fe = [ optim.Adam(self.FE[i].parameters(),
                                   lr=lr, weight_decay=0.0005) for i in range(self.N) ]
    ...
    def train(self, maxepoch):
        min_test_loss = 10
        save_path =  str(self.dataset) + '/' + 'ex' + str(self.ex_num) + '/' + 'user' + str(self.test_user) + '/'
        ...
        for epoch in range(maxepoch):
            ...
            for batch_idx, data in enumerate(self.datasets):
                ...
                """ step 1 """
                local_features = [ self.FE[i](img_s_list[i]) for i in range(self.N) ]
                local_preds = [ self.LD[i](local_features[i]) for i in range(self.N) ]
                concat_local_preds = torch.cat( [local_preds[i] for i in range(self.N)], 1) 
                ...
                loss_ld_s = [ criterion(local_preds[i], label_s_st) for i in range(self.N) ]
                loss_ld_s = torch.mean( torch.stack( [loss_ld_s[i] for i in range(self.N)], 0), dim=0)
                
                local_features = [ self.FE[i](img_t_list[i]) for i in range(self.N) ]         
                local_preds = [ self.LD[i](local_features[i]) for i in range(self.N) ]        
                concat_local_preds = torch.cat( [local_preds[i] for i in range(self.N)], 1) 
                weights = self.WG(local_features, concat_local_preds)
                weighted_features = torch.zeros(local_features[0].shape[0], local_features[0].shape[1], local_features[0].shape[2]).cuda()
                ...
                output_t1 = self.AC(feat_t)
                output_td = self.GD(feat_t)
                loss_ld_t = [ criterion(local_preds[i], label_t_st) for i in range(self.N) ]
                loss_ld_t = torch.mean( torch.stack( [loss_ld_t[i] for i in range(self.N)], 0), dim=0)
